...
def generate_maps(pool = None, path='SME_and_Lindblad_maps/'):
    logfile_name = path + 'log.txt'
    i = 0
    s = SME_scheme(logfile_name=logfile_name)
    ...
    try:
        with open(path+f'SME_Kraus_ops_{int(2*s.order)}.pkl', 'rb') as file:
            Mmus = pickle.load(file)
        log_write(f"SME Kraus operators of order {int(2*s.order)} was already saved", logfile_name=logfile_name)
    except FileNotFoundError as e:
        log_write(f"Computing SME Kraus operators of order {int(2*s.order)}...", logfile_name=logfile_name)
        Mmus = SME_Kraus_ops(s)
        log_write(f"SME Kraus operators of order {int(2*s.order)} was saved", logfile_name=logfile_name)
        with open(path+f'SME_Kraus_ops_{int(2*s.order)}.pkl', 'wb') as file:
            pickle.dump(Mmus, file, protocol=pickle.HIGHEST_PROTOCOL)
        log_write(f'SME_Kraus_ops_{int(2*s.order)}.pkl saved', logfile_name=logfile_name)
    ...
    while True:
        ...
        log_write(f"Computing SME Kraus operators of order {int(2*s.order)}...", logfile_name=logfile_name)
        Mmus = SME_Kraus_ops(s, pool=pool)
        with open(path+f'SME_Kraus_ops_{int(2*s.order)}.pkl', 'wb') as file:
            pickle.dump(Mmus, file, protocol=pickle.HIGHEST_PROTOCOL)
        log_write(f"SME Kraus operators of order {int(2*s.order)} saved", logfile_name=logfile_name)

    
if __name__ == "__main__":
    path  = 'SME_and_Lindblad_maps/'
    os.makedirs(path, exist_ok=True)
    log_write(f"Running on {os.cpu_count()} CPU cores", init = False, logfile_name=path+'log.txt')
    p = Pool(processes=os.cpu_count())
    generate_maps(pool=None, path = path)
